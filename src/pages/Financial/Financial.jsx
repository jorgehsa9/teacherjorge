...
const TeacherFinancial = () => {
  ...
  const handleUpdatePaymentStatus = async (studentEmail, newStatus) => {
    if (!activeMonthData) return;
    setIsSubmitting(true);
    
    const studentData = activeMonthData.studentsData.find(s => s.email === studentEmail);
    ...
    if (existingPayment) {
      ...
      const { error } = await supabase.from('Payments').insert([{
        ...
        amount: amount,
        reference_month: activeMonthData.refMonthStr,
        status: newStatus,
        paid_at: newStatus === 'Pago' ? new Date().toISOString() : null
      }]);
      errorObj = error;
    ...
    <div>
      {monthsData.map((monthData, index) => (
        <div key={monthData.refMonthStr} className="mb-12">
          {index === 0 && <h2 className="text-2xl font-bold mb-6 text-white border-b border-white/10 pb-4">Mês Atual: {monthData.monthLabel}</h2>}
          {index > 0 && <h2 className="text-xl font-bold mb-6 text-muted border-b border-white/10 pb-4 capitalize">{monthData.monthLabel}</h2>}
              ...
              <div style={{ marginBottom: '1.5rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  {activeStudent.classes.length > 0 ? activeStudent.classes.map(cls => (
                    <div key={cls.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '8px' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        <span style={{ fontWeight: 'bold', fontSize: '0.9rem' }}>{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(classPrice)}</span>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: '0.75rem' }}>
                  <button 
                    ...
                    onClick={() => handleUpdatePaymentStatus(activeStudent.email, 'A Receber')}
                  ...
                  <button 
                    ...
                    onClick={() => handleUpdatePaymentStatus(activeStudent.email, 'Pago')}
                    disabled={isSubmitting}
                  >
                    Pago
                  </button>
